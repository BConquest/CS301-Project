...
class page_DelivererInfo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.lbl_Title = Label(self, text="Deliverer Account Information")
        ...
        self.lbl_Error = Label(self, textvariable=self.txt_Error)
        ...
        self.lbl_Error.grid(row=3,column=2,columnspan=2)
        self.btn_Back.grid(row=4,column=0)
        self.btn_Delete.grid(row=4,column=2)
        self.btn_Update.grid(row=4,column=4)
    ...
    def update(self):
        ...
        try:
            ...
            print("Error Updatign person: " + err)
            return
        self.controller.show_frame("page_DelivererInfo")

    def delete(self):
        cnx = mysql.connector.connect(user='root', password='', host='127.0.0.1', database='grocerybama')
        cursor = cnx.cursor(buffered=True)
